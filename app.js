...
const updatePage = async () => {
  ...
  const highSugar = fruitsArray.filter(fruit => fruit.nutritions.sugar > 15).map(fruit => " " + fruit.name)
  console.log(highSugar)

  // TODO: Create a new HTML element to display your data
  var sugarElement = document.createElement('div');
  sugarElement.innerHTML = '<h2>The Very Sugary Fruits</h2>';
  sugarElement.innerHTML += highSugar;

  // TODO: Append your new element to the page
  const existingElement = document.getElementById('list')
  existingElement.append(sugarElement)
